...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('kod')
                .setDescription('Code name (if auto bot will automatically generate new code name')
                .setRequired(true))
        .addStringOption(option =>
            option.setName('rola')
    ...
    if(interaction.member.roles.cache.some(role => role.id === '1060912823187816549')){
        ...
        const codeValue = interaction.options.getString('kod') ?? genRandomString()
        const roles = [getIdFromChoice(interaction.options.getString('rola')),
        ...
        if(await codesCollection.findOne({value: codeValue, isActive: true})){
            interaction.reply(({
                embeds: [
                    {
                        "type": "rich",
            ...
            await codesCollection.insertOne({value: codeValue, numberOfUse: 0, maxNumberOfUse: maxUse, roleIds: roles, createdBy: interaction.user, isActive: true, usedBy: [], timeCreated: new Date()})

            let descriptionRoles = ""

            for(let i = 0; i < roles.length; i++){
                if(roles[i])
                    descriptionRoles += "<@&" + roles[i] + "> "
            }
                ...
                embeds: [
                    {
                        "type": "rich",
                        "title": "Sukces! ✅",
                        "description": `Pomyślnie utworzono nowy kod \`${codeValue}\` \nRole które zostaną nadane użytkownikowi po dołączeniu:` + descriptionRoles,
                        "color": 0x008000,
                        "image": {
                            "url": ``,
                            "height": 0,
