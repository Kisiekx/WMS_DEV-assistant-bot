...
const wmsRoleId = "1060911991142764587";

module.exports = {
  name: Events.MessageCreate,
  async execute(message) {
    if (message.guild) return;
    let process = await processesCollection.findOne({
      ...
      if (process.inProcess) {
        ...
        collector.on("collect", async (m) => {
          if (m.content.toUpperCase() === captcha.value) {
            ...
            let currentCode = await codesCollection.findOne({
            ...
            if (currentCode == null) {
                ...
                embeds: [
                  {
                    ...
                    description:
                      "W ciągu 5 sekund powinienieś otrzymać pełen dostęp do serwera",
                    footer: {
                      text: "Wszelkie problemy z botem proszę zgłaszać do: Kisiek#2137",
                    },
                  },
                ],
              });
              ...
              let currentUsedBy = currentCode.usedBy.concat([
                message.author,
                new Date(),
              ]);
              ...
              await server.members.cache
                .get(message.author.id)
                .roles.add(server.roles.cache.get(wmsRoleId));

              for (let i = 0; i < currentCode.roleIds.length; i++) {
                if (currentCode.roleIds[i]) {
                  try {
                    console.log(currentCode.roleIds[i]);
                    let member = await server.members.cache.get(
                      message.author.id
                    );
                    let role = await server.roles.cache.get(
                      currentCode.roleIds[i]
                    );
                ...
                embeds: [
                  {
                    ...
                    title: "❌ ERROR ❌",
                    description:
                      "Niezidentyfikowany error, proszę o kontakt z administratorem: **Kisiek#2137**",
                  },
                ],
                components: [
                  new ActionRowBuilder().setComponents(
            ...
            !(m.content.toUpperCase() === captcha.value)
          ) {
            console.log(
              "TUTAJ KODY:",
              m.content.toUpperCase(),
